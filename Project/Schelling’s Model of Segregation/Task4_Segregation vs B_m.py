...
rho = 0.9
runs_per_Bm = 5
Bm_values = np.linspace(0.1, 0.6, 6)
...
for Bm in Bm_values:
    ...
    for _ in range(runs_per_Bm):
        grid = run_schelling(N, Bm, rho)
        s_total += compute_segregation(grid)
    avg_s = s_total / runs_per_Bm
    seg_values.append(avg_s)
    print(f"Bm={Bm:.2f} → ⟨s⟩ = {avg_s:.3f}")

plt.figure(figsize=(8, 5))
plt.plot(Bm_values, seg_values, marker='d')
plt.title(f"Segregation Coefficient ⟨s⟩ vs Tolerance $B_m$ (ρ={rho})")
