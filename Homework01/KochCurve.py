...
def iterate_koch(points):  # Defining a function named iterate_koch that takes the current curve as input
    ...
    for i in range(len(points) - 1):  # Looping over each segment of the current curve. The curve is defined by a sequence of points, and each pair of consecutive points forms a segment.
        ...
        angle = np.radians(60)  # Converting 60 degrees to radians because NumPy trigonometric functions use radians
...
points = np.array([[0, 0], [1, 0]])  # Initialising the curve with a single line segment defined by two points: (0,0) and (1,0)

iterations = 10  # Adjusted as needed

fig, ax = plt.subplots(figsize=(8, 4))  # Creating a new figure and a set of subplots
fig.suptitle("Koch Curve: Click on the plot for the next iteration")  # Setting a title for the plot that instructs the user about interactivity

ax.plot(points[:, 0], points[:, 1], color='blue')  # Plotting the initial line using the x and y coordinates from the points array
ax.set_aspect('equal')  # Ensuring that the x and y scales are equal, so the line is not distorted
ax.set_xticks([])  # Removing any tick marks on the x-axis
ax.set_yticks([])  # Removing any tick marks on the y-axis
ax.set_title("Iteration 0", fontsize=12)  # Setting a title for the initial line displayed on the plot
plt.draw()  # Redrawing the plot so that the initial line and title are updated on the screen.
plt.waitforbuttonpress()  # Pausing the execution until the user clicks on the plot or presses a key

for i in range(1, iterations + 1):  # Starting an iterative process to build the Koch curve step by step from the initial line

    points = iterate_koch(points)  # Generating the next iteration of the Koch curve by processing the current set of points

    ax.clear()  # Clearing the current axes to prepare for drawing the updated curve

    ax.plot(points[:, 0], points[:, 1], color='blue')  # Plotting the updated Koch curve using the x and y coordinates from the points array

    ax.set_aspect('equal')  # Ensuring that the x and y scales are equal, so the curve is not distorted
    ...
    plt.draw()  # Redrawing the plot so that the new curve and title are updated on the screen.
    plt.waitforbuttonpress()  # Pausing the execution until the user clicks on the plot or presses a key
